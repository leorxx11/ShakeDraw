...
- **体感跟随（视差）**：随设备轻微倾斜产生位移和 3D 倾斜效果（默认开启，可在设置页开关与调节强度；仅在结果界面前台运行；尊重“降低动态效果”）
- **交互范围可调**：设置拖拽幅度（相对屏幕比例）与缩放范围（最小/最大倍率），边界触达提供触觉反馈
- **智能分享扩展**：
...
#### 体感跟随（可选）
- 设置页 ->「体感跟随」：开启/关闭与强度调节（0%~100%）
- 不会触发摇一摇，仅在结果图片展示时生效；若系统开启“降低动态效果”，将自动停用

#### 交互范围（可选）
- 设置页 ->「交互范围」：拖拽幅度（屏幕的20%~50%）与缩放范围（×0.5~×3.0）
...
### v1.5.0 (2025-08-20)
- 🆕 体感跟随（视差）：基于 CoreMotion 的姿态/重力数据，结果图片跟随设备轻微倾斜产生位移与 3D 倾斜
  - 设置页可开关与调节强度；仅前台展示时运行，减少耗电
  - 尊重“降低动态效果”，自动禁用防眩晕
