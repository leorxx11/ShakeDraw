...
### 🔧 高级功能
- **奖池管理**：设置 →「奖池」中导入多个文件夹、单独控制参与抽签，支持左划删除；内置「收藏」文件夹（来自分享扩展）
...
#### 收藏管理
- 入口：设置 →「奖池」→ 点击“收藏”卡片，或长按“收藏”卡片在菜单中选择“管理收藏”。
- 功能：支持多选删除、导出到相册、全选/取消全选；顶部显示图片数量与占用空间统计。
- 清空：设置 →「奖池」→ 长按“收藏”卡片 → 选择“清空收藏”。
...
- 收藏：App Group（`group.com.leorxx.ShakeDraw`）接收分享扩展图片，出现在“奖池”中，可单独参与/管理。
